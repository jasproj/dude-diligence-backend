  ...
  try {
    ...
    for (const entity of entitiesToCheck) {
      ...
      if (entity.type === 'company') {
        ...
        // Florida Sunbiz (Florida Secretary of State) - check if entity appears to be Florida-based
        const entityNameUpper = entity.name.toUpperCase();
        const isFloridaEntity = entityNameUpper.includes('FLORIDA') || 
                               entityNameUpper.includes(' FL ') ||
                               entityNameUpper.endsWith(' FL') ||
                               (countryCode === 'US' && (entity.country?.toUpperCase().includes('FLORIDA') || entity.country?.toUpperCase() === 'FL'));
        
        // Also check if it's a US company - we can still search Sunbiz for any US company
        const shouldCheckSunbiz = isFloridaEntity || countryCode === 'US';
        
        if (shouldCheckSunbiz) {
          const sunbizResult = await checkFloridaSunbiz(entity.name);
          if (!results.databasesChecked.includes('Florida Sunbiz')) {
            results.databasesChecked.push('Florida Sunbiz');
          }
          results.floridaSunbiz.searched = true;
          
          if (sunbizResult.found) {
            results.floridaSunbiz.found = true;
            results.floridaSunbiz.entities.push({
              searchedName: entity.name,
              ...sunbizResult
            });
            results.floridaSunbiz.totalResults += sunbizResult.matches?.length || 1;
            results.riskScore -= 10;
            results.positiveSignals.push('✓ ' + entity.name + ': Verified in Florida Sunbiz (Secretary of State)');
          }
        }
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      data.results.slice(0, 3).forEach((r, i) => {
        console.log('[OpenSanctions] Result ' + (i+1) + ': "' + r.caption + '" score=' + r.score + ' datasets=' + (r.datasets?.join(',') || 'none'));
      });
    }
    ...
    if (data.results && data.results.length > 0) {
      ...
      const significantMatches = data.results.filter(r => {
        ...
        if (isHighPriorityList) {
          console.log('OpenSanctions HIGH-PRIORITY: ' + r.caption + ' (score: ' + r.score + ', datasets: ' + (r.datasets?.join(', ') || 'none') + ')');
          return true;
        }
...
async function checkInterpolRedNotices(name) {
  try {
    ...
    const url = 'https://ws-public.interpol.int/notices/v1/red?forename=' + encodeURIComponent(forename) + '&name=' + encodeURIComponent(surname) + '&resultPerPage=20';
...
async function checkInterpolYellowNotices(name) {
  try {
    ...
    const url = 'https://ws-public.interpol.int/notices/v1/yellow?forename=' + encodeURIComponent(forename) + '&name=' + encodeURIComponent(surname) + '&resultPerPage=10';
...
async function checkWorldBankDebarred(name) {
  try {
    const url = 'https://finances.worldbank.org/resource/kvtn-9wxx.json?$q=' + encodeURIComponent(name) + '&$limit=10';
...
async function checkTradeGovCSL(name) {
  try {
    const url = 'https://api.trade.gov/gateway/v1/consolidated_screening_list/search?q=' + encodeURIComponent(name) + '&limit=10';
...
async function checkSAMGovExclusions(name) {
  try {
    ...
    const url = 'https://api.sam.gov/entity-information/v3/entities?q=' + encodeURIComponent(name) + '&exclusionStatus=Active&api_key=DEMO_KEY&limit=10';
...
async function checkSingaporeACRA(companyName) {
  try {
    ...
    const url = 'https://api.opencorporates.com/v0.4/companies/search?q=' + encodeURIComponent(companyName) + '&jurisdiction_code=sg&per_page=5';
...
async function checkSECEdgar(companyName) {
  try {
    ...
    const url = 'https://efts.sec.gov/LATEST/search-index?q=' + encodeURIComponent(companyName) + '&dateRange=custom&startdt=2020-01-01&enddt=' + today + '&forms=10-K,10-Q,8-K';
...
async function checkDomainAge(domain) {
  try {
    ...
    const rdapResponse = await fetch(url, {
...
// ============================================
// FLORIDA SUNBIZ (Secretary of State) CHECK
// ============================================

async function checkFloridaSunbiz(companyName) {
  console.log('[Florida Sunbiz] Checking: "' + companyName + '"');
  try {
    // Clean company name for search - remove common suffixes for better matching
    let searchTerm = companyName.trim();
    
    // URL encode the search term
    const encodedSearch = encodeURIComponent(searchTerm);
    
    // Sunbiz search URL
    const searchUrl = 'https://search.sunbiz.org/Inquiry/CorporationSearch/SearchResults?inquiryType=EntityName&searchTerm=' + encodedSearch + '&searchNameOrder=';
    ...
    console.log('[Florida Sunbiz] Response status: ' + response.status);

    if (!response.ok) {
      console.log('[Florida Sunbiz] HTTP error: ' + response.status);
      return { found: false, error: 'HTTP ' + response.status, matches: [] };
    }

    const html = await response.text();
    
    // Parse the HTML response to extract company data
    // Sunbiz returns a table with columns: Entity Name, Document Number, Status, etc.
    
    const results = [];
    
    // Look for table rows with company data
    // Pattern: <a href="/Inquiry/CorporationSearch/SearchResultDetail?...">COMPANY NAME</a>
    const linkPattern = /<a[^>]*href="\/Inquiry\/CorporationSearch\/SearchResultDetail[^"]*"[^>]*>([^<]+)<\/a>/gi;
    const statusPattern = /<td[^>]*>([^<]*(?:Active|Inactive|Dissolved|Revoked|Withdrawn)[^<]*)<\/td>/gi;
    
    let match;
    const companyLinks = [];
    
    // Extract all company name links
    ...
    // Also try a more specific pattern for the results table
    // Sunbiz uses a specific table structure
    ...
    while ((rowMatch = rowPattern.exec(html)) !== null) {
      ...
      if (cells.length >= 3) {
        // Typical structure: Entity Name, Document Number, Status, Filing Date
        const entityName = cells[0];
        const docNumber = cells[1];
        const status = cells[2];
        const filingDate = cells[3] || null;
        
        // Check if this is a relevant match
        const searchNameNormalized = companyName.toUpperCase().replace(/[^A-Z0-9]/g, '');
        const entityNameNormalized = entityName.toUpperCase().replace(/[^A-Z0-9]/g, '');
        
        // Match if names are similar (contains or high overlap)
        if (entityNameNormalized.includes(searchNameNormalized) || 
            searchNameNormalized.includes(entityNameNormalized) ||
            calculateSimilarity(searchNameNormalized, entityNameNormalized) > 0.7) {
          results.push({
    ...
    // If table parsing didn't work, try alternative pattern
    if (results.length === 0 && companyLinks.length > 0) {
      // Filter links that match our search
      const searchNameNormalized = companyName.toUpperCase().replace(/[^A-Z0-9]/g, '');
      
      for (const link of companyLinks) {
        const linkNameNormalized = link.name.toUpperCase().replace(/[^A-Z0-9]/g, '');
        
        if (linkNameNormalized.includes(searchNameNormalized) || 
            searchNameNormalized.includes(linkNameNormalized) ||
            calculateSimilarity(searchNameNormalized, linkNameNormalized) > 0.7) {
          results.push({
    ...
    // Alternative: Check if we got "No results" message
    const noResultsPattern = /No\s+(?:Records|Results|Entities)\s+Found/i;
    if (noResultsPattern.test(html) || html.includes('Your search did not find any')) {
      console.log('[Florida Sunbiz] No results found for: ' + companyName);
      return { found: false, matches: [], message: 'No matching Florida entities found' };
    }
    
    // If we still have no results but also no "no results" message, 
    // check if the search term appears anywhere in active results
    if (results.length === 0) {
      const activePattern = new RegExp(companyName.replace(/[^A-Za-z0-9\s]/g, '').split(/\s+/).join('.*'), 'i');
      if (activePattern.test(html) && html.includes('Active')) {
        // Likely found but couldn't parse properly
        results.push({
          entityName: companyName,
          documentNumber: 'See Sunbiz',
          status: 'Likely Active',
          filingDate: null,
          source: 'Florida Sunbiz',
          note: 'Match found - verify at sunbiz.org'
        });
      }
    }

    if (results.length > 0) {
      console.log('[Florida Sunbiz] Found ' + results.length + ' matches for: ' + companyName);
      return {
        found: true,
        matches: results,
        bestMatch: results[0],
    ...
    console.log('[Florida Sunbiz] No matches for: ' + companyName);
    return { found: false, matches: [], message: 'No matching Florida entities found' };

  } catch (error) {
    console.error('[Florida Sunbiz] Error:', error.message);
    return { found: false, error: error.message, matches: [] };
  }
}
